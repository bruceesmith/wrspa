...
function on_check(msg) {
  return on("change", subfield(toList(["target", "checked"]), bool, (checked2) => {
    return success(msg(checked2));
  }));
}
// build/dev/javascript/muster/muster/button.mjs
...
function variant_fragment(variant) {
  if (variant instanceof Elevated) {
    return "-elevated";
  } else if (variant instanceof Filled) {
    return "-filled";
  } else if (variant instanceof Outlined) {
    return "-outlined";
  } else if (variant instanceof Text2) {
    return "-text";
  } else {
    return "-tonal";
  }
}
function button2(size2, variant, extra_classes, attributes, children) {
  let micl_class = "micl-button" + variant_fragment(variant) + size_fragment(size2);
  let _block;
  let _pipe = prepend(micl_class, extra_classes);
  _block = join(_pipe, " ");
  let all_classes = _block;
  return button(prepend(class$(all_classes), attributes), children);
}
...
class Ghost extends CustomType {
}
class Light extends CustomType {
}
class Solid extends CustomType {
}
...
function colour_classes(group, variety) {
  ...
  } else if (variety instanceof Solid) {
    if (group instanceof Primary) {
...
function chip(variety, size2, colour, attributes, children) {
  let attr = prepend(class$("gap-2 rounded-full transition-all"), prepend(size_classes(size2), prepend(colour_classes(colour, variety), attributes)));
  return div(append(variant_classes(variety), attr), children);
}

// build/dev/javascript/wrspa/input.mjs
class Flat2 extends CustomType {
}
class Light2 extends CustomType {
}
...
function input2(variety, size2, colour, attributes) {
  let attr = prepend(class$("disabled:opacity-50 disabled:cursor-not-allowed transition-all"), prepend(size_classes2(size2), prepend(colour_classes2(colour, variety), attributes)));
  return input(append(variant_classes2(variety), attr));
}

// build/dev/javascript/wrspa/switch.mjs
function switch$(set, label2, on_check2) {
  return div(toList([class$("grid grid-rows-1 grid-cols-2")]), toList([
    text3(label2),
    div(toList([
      class$("relative inline-block w-[52px] h-[32px] justify-self-center self-center")
    ]), toList([
      input(toList([
        checked(set),
        id("switch-component"),
        type_("checkbox"),
        class$("peer appearance-none w-[52px] h-[32px] bg-surface-container-highest rounded-full checked:bg-primary cursor-pointer transition-colors duration-300"),
        on_check(on_check2)
      ])),
      label(toList([
        for$("switch-component"),
        class$("absolute top-[4px] left-[2px] w-[24px] h-[24px] bg-outline peer-checked:bg-on-primary-container rounded-full border border-slate-300 shadow-sm transition-transform duration-300 peer-checked:translate-x-[25px] peer-checked:border-slate-800 cursor-pointer")
      ]), toList([]))
    ]))
  ]));
}
...
function custom2(start5, goal2, goal_error, start_error) {
  ...
  return div(toList([class$("grid grid-rows-[1fr_3fr_1fr] gap-2")]), toList([
    ...
    div(toList([
      ...
      input2(new Outlined3, new Large2, new PrimaryContainer, toList([
    ...
    div(toList([class$("justify-self-center")]), toList([
      button2(new Medium, new Elevated, toList([]), toList([
...
function random3(start5, goal3, rsvp_error) {
  ...
  return div(toList([class$("grid grid-rows-5")]), toList([
    ...
    div(toList([
      ...
      button2(new Medium, new Elevated, toList(["self-center"]), toList([on_click(new RandomEndPointsDisplayed)]), toList([text3("Continue")])),
      button2(new Medium, new Elevated, toList(["self-center"]), toList([on_click(new RedrawRandom)]), toList([text3("Deal again")]))
    ]))
  ]));
}
function choosing(state, endpoints, goal_error, start_error, rsvp_error) {
  ...
  return toList([
    div(toList([class$("grid grid-cols-2 gap-4 justify-center")]), toList([
      chip(new Outlined2, new Large2, new PrimaryContainer, toList([
...
function playing_controls(state, nav) {
  ...
  return div(toList([class$("grid lg:px-50 md:px-25 sm:px-5 gap-1 justify-center"), cols]), toList([
    button2(new Medium, new Elevated, toList([]), toList([on_click(new NavigateBack), back_disablement]), toList([text3("Back")])),
    button2(new Medium, new Elevated, toList([]), toList([on_click(new NavigateForward), fwd_disablement]), toList([text3("Forward")])),
    (() => {
      if (state instanceof Playing) {
        return button2(new Medium, new Elevated, toList([]), toList([on_click(new GamePaused)]), toList([text3("Pause")]));
      } else if (state instanceof Paused) {
        return button2(new Medium, new Elevated, toList([]), toList([on_click(new GameResumed)]), toList([text3("Continue")]));
      } else if (state instanceof ReadyToPlay) {
        return button2(new Medium, new Elevated, toList([]), toList([on_click(new GameStarted)]), toList([text3("Play")]));
      } else {
        return none2();
      }
    })(),
    (() => {
      if (state instanceof Completed) {
        return button2(new Medium, new Elevated, toList([]), toList([on_click(new NewGame)]), toList([text3("New Game")]));
      } else if (state instanceof Paused) {
        return button2(new Medium, new Elevated, toList([]), toList([on_click(new NewGame)]), toList([text3("New Game")]));
      } else {
        return none2();
    ...
    (() => {
      if (state instanceof Paused) {
        return button2(new Medium, new Elevated, toList([]), toList([on_click(new RestartGame)]), toList([text3("Restart")]));
      } else {
        return none2();
      }
    })()
...
function view(model) {
  ...
  let body = _block;
  let title = div(toList([class$("grid grid-rows-1 grid-cols-[19fr_1fr] border-none")]), toList([
    p(toList([
      class$("font-bold italic pb-1 pt-1 justify-self-center text-primary text-3xl")
